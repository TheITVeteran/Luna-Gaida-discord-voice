...
const envSchema = z.object({
  ...
  YTDLP_PLAYER_CLIENTS: z.string().default('android,web,ios,tv,mweb'),
  LUNA_YT_DEFAULT_FORMAT: z.string().default('bestaudio[ext=m4a]/bestaudio/best'),
  ...
  LUNA_SEARCH_PROVIDER: z.enum(['duckduckgo', 'searxng']).default('duckduckgo'),
  ...
  LUNA_SELF_CONCEPT: z.enum(['true', 'false']).default('true').transform((value) => value === 'true'),
  LUNA_GOALS: z.enum(['true', 'false']).default('true').transform((value) => value === 'true'),
  LUNA_OPINIONS: z.enum(['true', 'false']).default('true').transform((value) => value === 'true'),
  LUNA_DAILY_REFLECTION: z.enum(['true', 'false']).default('true').transform((value) => value === 'true'),
  LUNA_REFLECTION_MIN_HOURS: z.coerce.number().min(6).max(168).default(20),
  LUNA_REFLECTION_MAX_HOURS: z.coerce.number().min(8).max(168).default(28),
  ...
  LUNA_VIBE_LISTEN_SEC: z.coerce.number().int().min(3).max(30).default(10),
  ...
  LUNA_DM_TTS: z.enum(['true', 'false', '1', '0']).optional(),
  ...
  LUNA_RESEARCH_MAX_READ_CHARS: z.coerce.number().int().min(1000).default(12000),
  LUNA_DEEP_RESEARCH: z.enum(['true', 'false', '1', '0']).optional(),
  LUNA_DEEP_RESEARCH_PAGES: z.coerce.number().int().min(1).max(8).default(5),
  ...
  LUNA_VIDEO_VISION: z.enum(['true', 'false', '1', '0']).optional(),
  LUNA_VIDEO_VISION_FRAMES: z.coerce.number().int().min(1).max(5).default(3),
  LUNA_VIDEO_VISION_PROVIDER: z.enum(['ollama', 'nvidia']).default('ollama'),
  OLLAMA_VISION_MODEL: z.string().optional(),
  LUNA_ABSENCE_MISS_HOURS: z.coerce.number().min(1).max(168).default(3),
  LUNA_TRAINING_LOG: z.enum(['true', 'false', '1', '0']).optional(),
  LUNA_TRAINING_DATASET_DIR: z.string().default('./data/training'),
  ...
  FISH_AUDIO_DYNAMIC_VOICE: z.enum(['true', 'false', '1', '0']).optional(),
  FISH_AUDIO_REFERENCE_ID_SHOUT: z.string().optional(),
  FISH_AUDIO_REFERENCE_ID_WHISPER: z.string().optional(),
  FISH_AUDIO_REFERENCE_ID_SAD: z.string().optional(),
  FISH_AUDIO_REFERENCE_ID_EXCITED: z.string().default('58be4f99b95a45378a0a90179e4fe488'),
  FISH_AUDIO_REFERENCE_ID_HOSTILE: z.string().default('7b80df7aef1144b69644839e4e1426d9'),
...
export function loadConfig() {
  ...
  return {
    ...
    lunaVibeListenSec: parsed.LUNA_VIBE_LISTEN_SEC,
    lunaAutonomousDm: envFlag('LUNA_AUTONOMOUS_DM') ?? parsed.GIADA_VOICE_PROVIDER === 'local',
    lunaDmTtsEnabled: envFlag('LUNA_DM_TTS') ?? parsed.GIADA_VOICE_PROVIDER === 'local',
    ...
    lunaSearchProvider: parsed.LUNA_SEARCH_PROVIDER,
    ...
    lunaDeepResearch: envFlag('LUNA_DEEP_RESEARCH') ?? parsed.GIADA_VOICE_PROVIDER === 'local',
    lunaDeepResearchPages: parsed.LUNA_DEEP_RESEARCH_PAGES,
    ...
    lunaVideoVisionEnabled: envFlag('LUNA_VIDEO_VISION') ?? parsed.GIADA_VOICE_PROVIDER === 'local',
    lunaVideoVisionFrames: parsed.LUNA_VIDEO_VISION_FRAMES,
    lunaVideoVisionProvider: parsed.LUNA_VIDEO_VISION_PROVIDER,
    ollamaVisionModel: parsed.OLLAMA_VISION_MODEL?.trim() || ollamaModel,
    lunaAbsenceMissHours: parsed.LUNA_ABSENCE_MISS_HOURS,
    lunaDailyReflection: parsed.LUNA_DAILY_REFLECTION,
    lunaReflectionMinHours: parsed.LUNA_REFLECTION_MIN_HOURS,
    lunaReflectionMaxHours: Math.max(parsed.LUNA_REFLECTION_MAX_HOURS, parsed.LUNA_REFLECTION_MIN_HOURS + 2),
    lunaTrainingLog: envFlag('LUNA_TRAINING_LOG') ?? false,
    lunaTrainingDatasetDir: resolveProjectFile(parsed.LUNA_TRAINING_DATASET_DIR),
    nvidiaApiKey: parsed.NVIDIA_API_KEY?.trim() || envString('NVIDIA_API_KEY'),
    fishAudioDynamicVoice: envFlag('FISH_AUDIO_DYNAMIC_VOICE') ?? parsed.LUNA_TTS_PROVIDER === 'fish',
    fishAudioVoiceVariants: {
      shout: parsed.FISH_AUDIO_REFERENCE_ID_SHOUT?.trim() || undefined,
      whisper: parsed.FISH_AUDIO_REFERENCE_ID_WHISPER?.trim() || undefined,
      sad: parsed.FISH_AUDIO_REFERENCE_ID_SAD?.trim() || undefined,
      excited: parsed.FISH_AUDIO_REFERENCE_ID_EXCITED?.trim() || undefined,
      hostile: parsed.FISH_AUDIO_REFERENCE_ID_HOSTILE?.trim() || undefined
    },
